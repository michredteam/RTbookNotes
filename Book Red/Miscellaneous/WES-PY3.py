#!/usr/bin/env python
# -*- coding: utf-8 -*-
# This is a version of the classic Windows Exploit Suggester tool that has been modified to work with Python 3.
#
  ...
  for row in list(potential):
    ...
    for hotfix in hotfixes:
      ...
      if (hotfix == kb or hotfix == componentkb) and bulletinid in bulletinids:
        ...
        linkedms = getlinkedms([bulletinid], csv.reader(StringIO(database)))
        linkedmsstr = ''
  ...
  if ARGS.local:
    ...
    for row in list(potential):
      ...
      if bulletinid in bulletinids and not "elevation of privilege" in impact.lower():
        ...
        remove = getlinkedms([bulletinid], csv.reader(StringIO(database)))
    ...
    if msid not in alerted: 
      ...
      if ARGS.audit or (exploit == ALERT.MSF or exploit == ALERT.EXP):
        ...
        if ARGS.sub:
          ...
          linked = set(getlinkedms([msid], csv.reader(StringIO(database))))
...
def trace(database):
  ...
  # get linked msids
  lmsids =  getlinkedms([bulletinid], csv.reader(StringIO(database)))
